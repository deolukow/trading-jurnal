...
const getItemsByProfileId = (storeName, profileId) =>
  dbAction(storeName, "readonly", (store, resolve) => {
...
const SidebarLink = ({ icon, text, active, onClick }) => (
  <li className={`mb-2`}>
    <a
      href="#"
      onClick={onClick}
      className={`flex items-center p-3 rounded-lg transition-colors ${active ? "bg-blue-600 text-white" : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white"}`}
    >
      {icon}
          ...
          <div>
            ...
            <div className="flex space-x-2 p-1 bg-gray-100 dark:bg-gray-900 rounded-lg">
              <button
                ...
                className={`flex-1 py-2 text-sm rounded-md transition-colors ${goalType === "weekly" ? "bg-blue-600 text-white" : "text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
              >
                Mingguan
              </button>
              <button
                onClick={() => setGoalType("monthly")}
                className={`flex-1 py-2 text-sm rounded-md transition-colors ${goalType === "monthly" ? "bg-blue-600 text-white" : "text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"}`}
              >
                Bulanan
              </button>
            </div>
            <input
              ...
              placeholder={`Jumlah Target ${goalType === "weekly" ? "Mingguan" : "Bulanan"}`}
      ...
      >
        <div className="aspect-video w-full overflow-hidden bg-gray-100 dark:bg-gray-700">
          ...
          <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <button className="text-white bg-blue-600 p-3 rounded-full hover:bg-blue-700">
              <Info size={24} />
            </button>
          </div>
        </div>
        <div className="p-3">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1 flex justify-between items-center">
            {trade.pair}
            <span
              className={`text-xs font-medium px-2 py-1 rounded-full ${trade.type === "long" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300"}`}
            >
...
const GaugeChart = ({ value, size = 80, strokeWidth = 8 }) => {
  const center = size / 2;
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <span className="absolute inset-0 flex items-center justify-center text-gray-900 dark:text-white font-bold text-lg">{`\'${Math.round(value)}%`}</span>
    </div>
  );
};
const RatioBar = ({ winValue, lossValue }) => {
...
const StatCard = ({ title, value, children, footer, icon }) => (
...
const GoalProgress = ({ goal, currentPnl, period, currency }) => {
  if (
    !goal ||
    !goal.type ||
    !goal.amount ||
      ...
      <div className="flex items-center space-x-4">
        <div className="flex-grow">
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 relative overflow-hidden">
            <div
              className={`h-4 rounded-full transition-all duration-500 ease-out ${isAchieved ? "bg-green-500" : "bg-blue-500"}`}
        ...
        <div className="flex-shrink-0 w-40 text-right">
          <span
            className={`font-bold text-lg ${isAchieved ? "text-green-400 dark:text-green-300" : "text-gray-900 dark:text-white"}`}
          >
              ...
              >
                <div className="flex flex-col text-sm">
                  ...
                  <span className="text-gray-500 dark:text-gray-400 text-xs">
                    {t.pair || "-"} | {t.type} | Lot: {t.lotSize.toFixed(2)} |
                    R:R:{" "}
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-gray-900 dark:text-white capitalize">
                      {t.type}: {formatCurrency(t.amount, currency)}
                    </p>{" "}
                    <p className="text-xs text-gray-500 dark:text-gray-400">
            ...
            {trades.length === 0 ? (
              ...
              trades.map((trade) => (
                ...
                >
                  ...
                  <td className="p-3">
                    <span
                      className={`px-2 py-1 text-xs font-semibold rounded-full ${trade.type === "long" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300"}`}
                    >
                      {trade.type}
                    </span>
                  </td>
                  ...
                  <td
                    className={`p-3 text-right font-bold ${trade.pnl >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    {formatCurrency(trade.pnl, currency)}
                  </td>
              ...
              monthlyData.map((data) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className={`p-3 text-right font-bold \    {data.pnl >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                    >
...
const CalendarView = ({
      ...
      <div className="flex justify-between items-center p-4">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="mt-2 text-sm">
            ...
            <span
              className={`font-bold ${monthlyPnl >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
            >
  ...
  const selectedTrades = selectedDate
    ? (tradesByDate.get(formatDate(selectedDate))?.trades || []).sort(
        (a, b) => (b.tradeDate?.getTime() || 0) - (a.tradeDate?.getTime() || 0),
      )
    : [];
  return (
    <div className="animate-fadeIn">
      ...
      {selectedDate && (
        <TradeList
          ...
          onView={onView}
          onDelete={onDelete}
          title={`Trade untuk ${selectedDate.toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}`}
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <ChevronDown
          size={16}
          className={`transition-transform text-gray-600 dark:text-gray-400 ${isOpen ? "rotate-180" : ""}`}
        />
      </button>
      {isOpen && (
          ...
          <ul>
            ...
            <li>
              <button
                ...
                className="w-full text-left px-4 py-2 text-sm text-blue-500 dark:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700 border-t border-gray-200 dark:border-gray-700"
              >
                Tambah/Kelola Akun...
              </button>
            </li>
...
// --- FIELD PERFORMANCE TABLE COMPONENT ---
const FieldPerformanceTable = ({ trades, customFields }) => {
  ...
  const performanceData = useMemo(() => {
    ...
    trades.forEach((trade) => {
      let value = "";
      if (selectedField.startsWith("custom_")) {
            ...
            {performanceData.length === 0 ? (
              ...
              performanceData.map((data, index) => (
                ...
                >
                  ...
                  <td className="p-3 text-center">
                    <span
                      className={`px-2 py-1 rounded font-bold text-xs ${data.winrate >= 50 ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}
                    >
...
function App() {
  ...
  const performanceStats = useMemo(() => {
    ...
    const statsTrades = [...filteredTrades].sort(
      (a, b) => (a.tradeDate?.getTime() || 0) - (b.tradeDate?.getTime() || 0),
    );
